...
export function QueueActions({ id, code }: { id: string; code?: string }) {
  ...
  return <div style={queueStyles.recovery}>
    <div style={queueStyles.actions}>
      ...
      {task.changes.slice(task.cursor).some((change) => change.action === "reprocess" ? code === undefined : change.code === code) ? <ConfButton label="Cancel change" onConfirm={() => cancel(id, code)} requireConfirmation={false} showPrompt={false} variant="secondary" size="sm" style={queueStyles.button} /> : null}
    </div>
