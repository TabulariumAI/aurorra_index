...
export function EditIndexPanel({ apiGatewayUrl, authToken, batchCode, intervalMs, retryIntervalMs, retryLimit, request, onClose, onError, onReadyChange, onResource, workerClient }: EditIndexPanelProps) {
  ...
  async function submit() {
    ...
    try {
      await queueStoreApi.getState().enqueue({ batch: batchCode, session: request.session, segment: request.segment, data: JSON.stringify([{
        action: "update", explanation: `P ${fields.page.trim()}  ${fields.source}`,
  ...
  return <section aria-label="Edit index form" style={indexStyles.root}>
    <div inert={submitting} style={indexStyles.fields}>
      <IndexInput key={request.session} value={fields.index} aspects={aspects} segment={request.segment} selected={selected} onChange={(index) => setFields({ ...fields, index })} onSelect={setSelected} />
